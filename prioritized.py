    ...
    def find_solution(self):
        ...
        for i in range(self.num_of_agents):  # Find path for each agent
            ...
            for j in range(1, len(path)):
                for k in range(self.num_of_agents - 1, i, -1):
                    if not (({'agent': k, 'loc': [path[j]], 'timestep': j}) in constraints):
                        constraints.append({'agent': k, 'loc': [path[j]], 'timestep': j})
                    if not (({'agent': k, 'loc': [path[j], path[j-1]], 'timestep': j}) in constraints):
                        constraints.append({'agent': k, 'loc': [path[j], path[j-1]], 'timestep': j})
            for n in range(self.num_of_agents - 1, i, -1):
                if not (({'agent': k, 'loc': [path[-1]], 'timestep': [len(path), 'inf']}) in constraints):
                    constraints.append({'agent': k, 'loc': [path[-1]], 'timestep': [len(path), 'inf']})
